...
class Solver{
	...
	Boolean solve(ArrayList<Student>arr,int h,int m,int n,int currpos_k,int currpos_m,int currpos_n){
		...
		if(currpos_k==h&&currpos_m==m&&currpos_n==n || state[currpos_k][currpos_m][currpos_n].filled){
			return false;
		}
		...
		for(Student s : arr){
			...
			if(!s.is_seated() && canbefilled(currpos_k)){
				...
				s.seat();
				Boolean solved = solve(arr,h,m,n,next_pos[0],next_pos[1],next_pos[2]);
				if(!solved){
